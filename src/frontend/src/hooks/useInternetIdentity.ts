...
export function InternetIdentityProvider({
  ...
  // Track whether we found an existing authenticated session on init
  const wasAuthenticatedOnInit = useRef(false);
  ...
  const login = useCallback(() => {
    ...
    // If user already has a valid session, mark as success immediately
    if (
  ...
  const clear = useCallback(() => {
    ...
    void authClient
      .logout()
      .then(() => {
        wasAuthenticatedOnInit.current = false;
    ...
    void (async () => {
      try {
        ...
        if (isAuthenticated) {
          ...
          wasAuthenticatedOnInit.current = true;
          // Set status to success so the admin page recognises the stored session
          setStatus("success");
          return;
        }
      } catch (unknownError) {
